...
const Navigation = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'journey', 'about', 'projects', 'contact'];
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => (
              ...
              >
                {item.label}
                {activeSection === item.id && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary to-accent rounded-[1px]" />
                )}
              </Button>
            ))}
            <Button
            ...
            <div className="flex flex-col space-y-2">
              {navItems.map((item) => (
                <Button
                  ...
                  onClick={() => scrollToSection(item.id)}
                  className={`
                    justify-start text-foreground/80 hover:text-foreground hover:bg-accent/10
                    transition-all duration-200
                    ${activeSection === item.id 
